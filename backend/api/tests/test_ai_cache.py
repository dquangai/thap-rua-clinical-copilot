...
def test_second_ai_check_with_only_non_ai_field_changed_is_cache_hit_and_uses_zero_tokens(monkeypatch):
    ...
    assert second["meta"]["latency_ms"] == 0
    assert second["meta"]["estimated_cost_usd"] == 0
    assert second["meta"]["original_latency_ms"] == 25
    assert second["meta"]["original_cost_usd"] == 0.0025
