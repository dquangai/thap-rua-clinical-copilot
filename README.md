...
| `backend/api/.env` | API server: `MONGODB_URI`, `OPENAI_API_KEY` (lab analysis), `AI_JOB_WORKERS`… | `backend/api/.env.example` |
...
Demo nhanh: mở ứng dụng → chọn ca `SIM-005` (đái tháo đường thai kỳ) → "Tạo biên bản tư vấn" → "In / Xuất PDF" → "Kiểm tra hồ sơ (AI)".
